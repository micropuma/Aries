...
void kernel_gemm(input_buffer<int32_t, adf::extents<1024>>& __restrict in0, input_buffer<int32_t, adf::extents<1024>>& __restrict in1, input_cascade<acc80>& __restrict accin, output_buffer<int32_t, adf::extents<1024>>& __restrict out0) {
  ...
  for (unsigned int i=0;i<boundary_i;i++) 
    ...
    for (unsigned int j=0; j< boundary_j; j++)
      ...
      aie::accum<acc80,8> acc0 = readincr_v<8>(accin);
			aie::accum<acc80,8> acc1 = readincr_v<8>(accin);
