...
void kernel_ttm0(input_buffer<int32_t, adf::extents<A_SIZE>>& __restrict in0, input_buffer<int32_t, adf::extents<B_SIZE>>& __restrict in1, output_buffer<int32_t, adf::extents<C_SIZE>>& __restrict out0) {
  ...
  for (unsigned int i=0;i<boundary_i;i++) 
    ...
    for (unsigned int j=0; j< boundary_j; j++)
      ...
      aie::accum<acc80,8> acc0 = null_v8acc80();
			aie::accum<acc80,8> acc1 = null_v8acc80();
