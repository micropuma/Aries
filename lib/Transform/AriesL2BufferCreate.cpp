...
private:
  ...
  // _ _ x _
  // _ _ _ x
  bool dmaProcess(OpBuilder builder, FuncOp func, DmaOp dmaOp,
  ...
  bool dmaBroadCastProcess(OpBuilder builder, FuncOp func, 
                           DmaBroadcastOp broadcast,
                           unsigned& loadIdx, unsigned& storeIdx, unsigned& cnt,
                           SmallVector<AffineForOp, 6>& band){
    auto loc = builder.getUnknownLoc();
    ...
    MemRefType type = dyn_cast<MemRefType>(src.getType());
    SmallVector<Value> offsets = broadcast.getSrcOffsets();
    SmallVector<Value> sizes   = broadcast.getSrcSizes();
    SmallVector<Value> strides = broadcast.getSrcStrides();
    SmallVector<Value> srcOffsets = broadcast.getSrcOffsets();
    SmallVector<Value> srcSizes   = broadcast.getSrcSizes();
    SmallVector<Value> srcStrides = broadcast.getSrcStrides();
    SmallVector<Value> dstOffsets = broadcast.getDstOffsets();
    SmallVector<Value> dstSizes   = broadcast.getDstSizes();
    SmallVector<Value> dstStrides = broadcast.getDstStrides();
    // Create local buffer. Now only support DMA that has static size and stride
    SmallVector<int64_t, 4> sizesInt; 
    for(auto size : sizes){
      auto constantOp = dyn_cast<arith::ConstantOp>(size.getDefiningOp());
    ...
    for (auto func : mod.getOps<FuncOp>()) {
      ...
      auto flag = arrayForOp.walk([&](Operation* op){
        if(auto dmaOp = dyn_cast<DmaOp>(op)){
          if(!dmaProcess(builder, func, dmaOp, loadIdx, storeIdx, cnt, band))
            return WalkResult::interrupt();
          dmaOp.erase();
        }else if(auto broadcast = dyn_cast<DmaBroadcastOp>(op)){
          if(!dmaBroadCastProcess(builder, func, broadcast, loadIdx, storeIdx, 
                                  cnt, band))
            return WalkResult::interrupt();
          broadcast.erase();
        }
